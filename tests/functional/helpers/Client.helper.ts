...
export class FakeClientForTest {
  ...
  private password: string = 'FakeClient1234'
  private passwordConfirmation: string = 'FakeClient1234'
  ...
  public async validateClientEmail() {
    await User.query().where('id', this.userId).update({ is_valid_email: 1 })
  }

  public async deleteFakeClient() {
    await ClientProfile.query().where('user_id', this.userId).delete()
